...
def create_farm_habitat_ratio_fix_batch(db, radius):
    query = sa.text(f'''
        ...
        -- 농장 포인트가 주어졌을때, 인근에 존재하는 서식지 면적을 산출하는 쿼리
        ...
        FROM (
            SELECT
                f.farm_serial_no,
                ST_Area(ST_Intersection(t1.geom, f.geometry)) AS intersection_area
            FROM farm_buffer_5179 AS f
            JOIN (SELECT id, geom 
                  FROM asf.tb_habitat_geom_clean_asf
                  WHERE median_habitat_possibility >= 0.75) AS t1
              ON ST_Intersects(t1.geom, f.geometry)
        ) i1

...
class HDNN(nn.Module):
    def __init__(self):
        ...
        self.col_list = pd.DataFrame({'col': list(
            self.mc.bio_col + self.mc.bio_mother_pig_col
         )})
    ...
    def forward(self, x):
        ...
        # 1) 환경 클러스터 카테고리
        cluster_bio_col = list(self.col_list[self.col_list.col.isin(self.mc.bio_col)].index)
        cluster_mother_bio_col = list(self.col_list[self.col_list.col.isin(self.mc.bio_mother_pig_col)].index)
        cluster_bio_final_col = cluster_bio_col + cluster_mother_bio_col
        cluster_bio_vec = x[:, cluster_bio_final_col]

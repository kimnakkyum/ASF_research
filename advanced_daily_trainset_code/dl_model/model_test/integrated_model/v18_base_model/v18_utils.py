...
from advanced_daily_trainset_code.dl_model.model_test.integrated_model.v1_base_model.v1_visit_clustering import generate_name_tag
...
# 발생농장 위험도 히스토그램 시각화
def plot_risk_percent_histogram(df, prob, year_label, model_path, save_name):
    df = df.copy()
    df['prob'] = prob
    df['risk_rank'] = df.groupby('standard_date')['prob'].rank(method='min', ascending=False)
    df['total_per_date'] = df.groupby('standard_date')['prob'].transform('count')
...
# 클러스터 절대위험도 박스플롯 시각화
def plot_cluster_boxplot(df, prob, col_name, cluster_order, model_path, save_name):
    df = df.copy()
    df['prob'] = prob

    plt.figure(figsize=(12, 6))
    sns.boxplot(
        x=col_name,  # cluster가 아닌 실제 컬럼명 사용
        y='prob',
        data=df,
        palette='Set3',
        order=cluster_order
    ...
    plt.title(f'{col_name}별 위험도 확률 (prob) 분포', fontsize=14)
...
def plot_sorted_scatter_pos_ties(eval_df, types, evals, model_path, tie_precision=4, annotate_min=2):
    # 1) 정렬
    sorted_df = eval_df[['asf_occurrence_yn', 'prob']].sort_values('prob').reset_index(drop=True)
    x_all = sorted_df.index.to_numpy()
    y_all = sorted_df['prob'].to_numpy()

    # 2) 양성(yn=1)만 분리
    mask_pos = (sorted_df['asf_occurrence_yn'] == 1).to_numpy()
    x_pos = x_all[mask_pos]
    y_pos = y_all[mask_pos]

    # 3) 플롯
    plt.figure()
    plt.scatter(x_all, y_all, s=10, marker='o', label='All points')
    plt.scatter(x_pos, y_pos, s=5, marker='o', label='ai_occurrence_yn = 1', color = 'red')

    # 4) 양성끼리만 동점 그룹핑 (반올림 기준)
    probs_pos_rounded = np.round(y_pos, tie_precision)
    tie_map = {}
    for xv, yv_r in zip(x_pos, probs_pos_rounded):
        tie_map.setdefault(yv_r, []).append(xv)

    # 5) 개수 라벨: annotate_min 이상인 동점만 표시
    for yv_r, xs in tie_map.items():
        if len(xs) >= annotate_min:
            xm = float(np.median(xs))
            ym = float(yv_r)
            plt.annotate(str(len(xs)), xy=(xm, ym), xytext=(0, 6),
                         textcoords='offset points', ha='center', va='bottom', fontsize=8)

    # 6) 상위 10% 점선 (확률 높은 쪽 기준)
    cutoff_idx = int(len(x_all) * 0.9)  # 상위 10% 시작 위치
    plt.axvline(x=cutoff_idx, color='gray', linestyle='--', linewidth=1)
    ymin, ymax = plt.ylim()
    offset = (ymax - ymin) * 0.02  # y축 범위의 2%만큼 위
    plt.text(cutoff_idx, ymin + offset, 'Top 10%', rotation=0,
            va='bottom', ha='right', fontsize=8, color='gray')

    plt.title(f"train til-{types}_eval to-{evals} (sorted_scatter)")
    plt.xlabel('Sorted Index (ascending prob)')
    plt.ylabel('Probability')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    img_name = f"train til-{types}_eval to-{evals} (sorted_scatter)"
    plt.savefig(os.path.join(model_path, img_name), dpi=300)
    plt.close()
